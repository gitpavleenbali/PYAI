"""
Semantic Kernel Integration for PyAgent
========================================

Bridge between PyAgent and Microsoft's Semantic Kernel framework.
Import SK plugins/functions into PyAgent, or use PyAgent within SK.

Features:
- Import Semantic Kernel plugins as PyAgent skills
- Use SK's planning capabilities with PyAgent agents
- Export PyAgent functions as SK plugins
        ...
        if provider == "azure":
            ...
            kernel.add_service(
                OpenAIChatCompletion(
                    model_id=deployment or "gpt-4o-mini",
                    api_key=api_key
                )
            )
    ...
    *,
    ...
    try:
        ...
        if plugin_name in builtin_plugins:
            ...
            # Load from directory
            sk_plugin = kernel.import_plugin_from_prompt_directory(
                plugin_path,
                plugin_name
            )
            return SKPlugin(
