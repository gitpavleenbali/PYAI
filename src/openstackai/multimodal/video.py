...
class Video:
    ...
    @classmethod
    def from_file(
        cls, path: Union[str, Path], format: Optional[VideoFormat] = None, **metadata
    ) -> "Video":
    ...
    @classmethod
    def from_bytes(cls, data: bytes, format: VideoFormat = VideoFormat.MP4, **metadata) -> "Video":
    ...
    def extract_frames(self, count: int = 5, uniform: bool = True) -> List["Image"]:
        ...
        with tempfile.NamedTemporaryFile(suffix=f".{self.format.value}", delete=False) as tmp:
...
def load_video(
    source: Union[str, Path, bytes], format: Optional[VideoFormat] = None, **metadata
) -> Video:
