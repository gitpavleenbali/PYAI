...
from openstackai.blueprint.orchestrator import AgentRole, Orchestrator
...
class RouterPattern(Orchestrator):
    ...
    def __init__(self, classifier: Optional[Callable[[str], str]] = None, name: str = "Router"):
        super().__init__(name)
    ...
    async def run(self, task: str, **kwargs) -> Any:
        ...
        if not route_name or route_name not in self._routes:
            raise ValueError("No route found for task")
...
class MapReducePattern(Orchestrator):
    ...
    def __init__(
        self, mapper_agent: Any, reducer_agent: Any, num_workers: int = 3, name: str = "MapReduce"
    ):
        super().__init__(name)
    ...
    async def run(self, task: str, items: Optional[List[Any]] = None, **kwargs) -> Any:
        """Execute map-reduce"""
        items = items or [task]
        ...
        mapped_outputs = [r.content if hasattr(r, "content") else str(r) for r in map_results]
...
class SupervisorPattern(Orchestrator):
    ...
    def __init__(self, supervisor: Any, workers: Dict[str, Any], name: str = "Supervisor"):
        super().__init__(name)
