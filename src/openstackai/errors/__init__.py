...
"""
openstackai Errors Module

Provides a structured error hierarchy for openstackai.
...
Example:
    from openstackai.errors import ModelError, RateLimitError
...
class pyaiError(Exception):
    """Base exception for all openstackai errors.

    All openstackai exceptions inherit from this class, making it easy
    to catch any openstackai-related error.

    Example:
        try:
            agent.run("Hello")
        except pyaiError as e:
            print(f"openstackai error: {e}")
    """
...
class ConfigurationError(pyaiError):
...
class APIError(ModelError):
    ...
    def __init__(
        self, message: str, status_code: int = None, provider: str = None, response: dict = None
    ):
        super().__init__(
            message, {"status_code": status_code, "provider": provider, "response": response}
        )
...
class ValidationError(pyaiError):
...
class OutputValidationError(ValidationError):
    ...
    def __init__(self, message: str, content_type: str = None, output_preview: str = None):
        super().__init__(message, {"content_type": content_type, "output_preview": output_preview})
...
class ExecutionError(pyaiError):
...
class CodeExecutionError(ExecutionError):
    ...
    def __init__(self, message: str, code: str = None, line_number: int = None):
        super().__init__(
            message, {"code_preview": code[:100] if code else None, "line_number": line_number}
        )
        self.code = code
...
class HandoffError(pyaiError):
    ...
    def __init__(self, message: str, source_agent: str = None, target_agent: str = None):
        super().__init__(message, {"source": source_agent, "target": target_agent})
...
def wrap_error(original: Exception, context: str = None) -> pyaiError:
    """Wrap an external exception in a pyaiError.
    ...
    Returns:
        Appropriate pyaiError subclass
    """
    ...
    return pyaiError(message, {"original_type": type(original).__name__})
...
def is_retriable(error: Exception) -> bool:
    ...
    Example:
        from openstackai.errors import is_retriable
