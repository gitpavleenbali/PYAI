...
Inspired by OpenAI Agents SDK's handoff pattern but simplified to openstackai's philosophy.
...
from openstackai.easy.llm_interface import get_llm
...
class AgentTeam:
    ...
    def __post_init__(self):
        if not self.router_prompt:
            agent_list = "\n".join(f"- {a.name}: {a.instructions[:100]}..." for a in self.agents)
...
"""
    ...
    def route(self, message: str, *, allow_multi: bool = False, model: str = None) -> HandoffResult:
        """
        ...
        routing = llm.json(
            f"Route this request: {message}", system=self.router_prompt, temperature=0.1
        )
...
Synthesize these into a single, comprehensive response that combines the best insights."""
        ...
        return HandoffResult(
            final_output=final, agents_used=[a.name for a in self.agents], handoff_log=results
        )
...
def handoff(
    ...
    *,
    ...
    """
    get_llm(model=model) if model else get_llm()
...
def chain(
    agents: List[Agent], task: str, *, pass_output: bool = True, model: str = None
) -> HandoffResult:
    """
...
Continue building on this for the task: {task}"""
    ...
    return HandoffResult(
        final_output=current_input, agents_used=[a.name for a in agents], handoff_log=handoff_log
    )
