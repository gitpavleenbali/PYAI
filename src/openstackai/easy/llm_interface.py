...
def _log_llm_call(
    model: str, messages: List[Dict], response_content: str, duration_ms: float, usage: Dict
):
    """Log an LLM call to the tracer if enabled."""
    try:
        from openstackai.easy.trace import TraceEvent, _tracer
...
class LLMInterface:
    ...
    def __init__(self, model: str = None, provider: str = None, api_key: str = None, **kwargs):
        config = get_config()
    ...
    def json(
        ...
        if self.provider in ("openai", "azure"):
            response = self.complete(
                prompt, system=json_system, response_format={"type": "json_object"}, **kwargs
            )
        else:
