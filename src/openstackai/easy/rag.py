...
from openstackai.easy.summarize import _extract_content, _read_file
...
def index(
    source: Union[str, Path, List[str]], *, chunk_size: int = 1000, overlap: int = 200, **kwargs
) -> IndexedDocuments:
    """
    ...
    if isinstance(source, list):
        ...
        return IndexedDocuments(
            content=content, source="multiple_sources", chunk_size=chunk_size, overlap=overlap
        )
    ...
    return IndexedDocuments(
        content=content, source=source_name, chunk_size=chunk_size, overlap=overlap
    )
...
def from_text(text: str, question: str, **kwargs) -> str:
    """
    RAG from raw text.
...
def search(source, query, **kw):
    return index(source, **kw).search(query)
