...
Inspired by Strands' MCP support but with openstackai's simplicity.
...
def tool(name: str = None, description: str = None) -> Callable:
    ...
    def decorator(func: Callable) -> MCPTool:
        ...
        return MCPTool(
            name=tool_name, description=tool_desc.strip(), parameters=params, handler=func
        )
...
def server(
    name: str, *, version: str = "1.0.0", tools: List[Union[MCPTool, Callable]] = None
) -> MCPServer:
    """
...
def from_directory(path: str, *, pattern: str = "*.py", server_name: str = None) -> MCPServer:
    """
    ...
    return MCPServer(name=server_name or dir_path.name, tools=tools)
