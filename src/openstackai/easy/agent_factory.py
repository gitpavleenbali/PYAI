...
class SimpleAgent:
    ...
    def __call__(self, message: str, *, remember: bool = None, **kwargs) -> str:
        """
        ...
        if use_memory:
            ...
            # Get response
            response = llm.chat(self._messages, temperature=self.temperature, **kwargs)
            ...
            response = llm.complete(
                message, system=self.instructions, temperature=self.temperature, **kwargs
            )
...
def agent(
    ...
    **kwargs,
    ...
    return SimpleAgent(
        instructions=instructions, model=model, temperature=temperature, name=name, memory=memory
    )


# Convenience function for one-shot tasks
def run(task: str, *, as_agent: str = None, instructions: str = None, **kwargs) -> str:
    """
