...
class StreamingRunner:
    ...
    def __init__(self, max_turns: int = 10, max_time: float = 300.0, verbose: bool = False):
    ...
    async def execute_stream(self, agent: Any, input: str, **kwargs) -> AsyncIterator[StreamEvent]:
    ...
    async def _stream_agent(
        self, agent: Any, messages: List[Dict], **kwargs
    ...
    async def _run_agent_async(self, agent: Any, messages: List[Dict], **kwargs) -> Any:
        ...
        if hasattr(agent, "run_async"):
            ...
            return await loop.run_in_executor(
                None, lambda: agent.run(messages[-1]["content"], **kwargs)
            )
        elif callable(agent):
            if asyncio.iscoroutinefunction(agent):
                ...
                loop = asyncio.get_event_loop()
                return await loop.run_in_executor(
                    None, lambda: agent(messages[-1]["content"], **kwargs)
                )
        else:
    ...
    @classmethod
    async def stream(
        cls, agent: Any, input: str, max_turns: int = 10, max_time: float = 300.0, **kwargs
...
def stream_sync(
    agent: Any, input: str, max_turns: int = 10, max_time: float = 300.0, **kwargs
