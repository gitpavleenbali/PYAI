...
"""
openstackai CLI Main Entry Point

Provides CLI commands similar to Google ADK's `adk` CLI:
- openstackai run: Run an agent interactively
- openstackai eval: Evaluate agent on test cases
- openstackai serve: Start API server
- openstackai init: Initialize new project
"""
...
def load_agent_from_file(filepath: str) -> Any:
    ...
    if path.suffix in (".yaml", ".yml"):
        ...
        if hasattr(module, "agent"):
            ...
            raise ValueError(
                f"No agent found in {filepath}. Define 'agent = Agent(...)' in your file."
            )
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    Example:
        openstackai run my_agent.py
        openstackai run config.yaml --input "Hello"
    """
    print(f"🚀 openstackai CLI v{__version__}")
...
def cmd_eval(args: argparse.Namespace) -> int:
    ...
    Example:
        openstackai eval tests.evalset.json --agent my_agent.py
        openstackai eval tests/*.json --parallel
    """
    print(f"🧪 openstackai Evaluation CLI v{__version__}")

    try:
        # Load eval set
        from ..evaluation import EvalConfig, EvalSet, Evaluator
...
def cmd_serve(args: argparse.Namespace) -> int:
    ...
    Example:
        openstackai serve my_agent.py --port 8000
    """
    print(f"🌐 openstackai API Server v{__version__}")
...
def cmd_init(args: argparse.Namespace) -> int:
    """Initialize a new openstackai project.

    Example:
        openstackai init my-agent
        openstackai init . --template research
    """
    print(f"🎉 openstackai Project Initializer v{__version__}")
...
if __name__ == "__main__":
    ...
    print(result.output)
''',
            "requirements.txt": "openstackai>=0.2.0\n",
            "README.md": f"# {args.name}\n\nAI Agent built with openstackai.\n\n## Usage\n\n```bash\npyai run agent.py\n```\n",
        },
        "research": {
            "agent.py": '''"""Research Agent with Tools"""
from openstackai import Agent
from openstackai.easy import research, fetch, summarize
...
''',
                    ...
                    "test_cases": [
                        {
                            ...
                            "expected_contains": ["algorithm", "data", "learn"],
                        }
                    ],
                },
                indent=2,
            ...
            "requirements.txt": "openstackai>=0.2.0\nrequests\n",
            "README.md": f"# {args.name}\n\nResearch Agent built with openstackai.\n",
        },
    }
    ...
    print("   openstackai run agent.py")

    return 0


def cmd_info(args: argparse.Namespace) -> int:
    """Show openstackai information."""
    print(f"openstackai v{__version__}")
...
def create_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        prog="openstackai",
        description="openstackai CLI - Build and run AI agents",
        epilog="For more info: https://github.com/openstackai/openstackai",
    )
    parser.add_argument("-V", "--version", action="version", version=f"openstackai {__version__}")
    ...
    init_parser.add_argument(
        "-t", "--template", default="basic", choices=["basic", "research"], help="Project template"
    )
    init_parser.add_argument("-f", "--force", action="store_true", help="Overwrite existing")

    # Info command
    subparsers.add_parser("info", help="Show openstackai info")
