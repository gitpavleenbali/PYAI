"""
LangChain Integration for openstackai
==================================

Bridge between openstackai and LangChain ecosystems. Import LangChain tools,
chains, and agents into openstackai, or export openstackai components to LangChain.

Features:
- Import LangChain tools as openstackai skills
- Use LangChain chains within openstackai workflows
- Export openstackai agents as LangChain-compatible
...
Examples:
    ...
    >>> agent = openstackai.agent("researcher", tools=[search_tool])

    # Use a LangChain chain
    >>> chain = langchain.import_chain("llm_math_chain")
    >>> result = chain.run("What is 25 * 4?")

    # Export openstackai to LangChain
    >>> lc_agent = langchain.export_agent(my_pyai)
"""
...
@dataclass
class LangChainTool:
    """Wrapper for LangChain tools in openstackai."""
    ...
    def to_pyai_skill(self):
        """Convert to openstackai skill format."""
        from openstackai.easy.mcp import MCPTool

        return MCPTool(
            name=self.name, description=self.description, parameters={}, handler=self.func
        )


def import_tool(tool_name: str, *, api_key: str = None, **kwargs) -> LangChainTool:
    """
    Import a LangChain tool for use in openstackai.
    ...
    try:
        ...
        return LangChainTool(
            name=lc_tool.name, description=lc_tool.description, func=lc_tool.run, _lc_tool=lc_tool
        )
    except ImportError:
...
def import_chain(chain_type: str, *, llm: Any = None, **kwargs) -> Any:
    """
    Import a LangChain chain for use in openstackai.

    Args:
        chain_type: Type of chain (e.g., "llm_math", "sql_database")
        llm: LLM to use (uses openstackai default if not specified)
...
def export_agent(pyai_agent, *, tools: List = None) -> Any:
    """
    Export a openstackai agent as a LangChain agent.

    Args:
        pyai_agent: openstackai SimpleAgent instance
    ...
    try:
        ...
        # Convert openstackai to LangChain tool
        lc_tools = []
        if tools:
            for t in tools:
                lc_tools.append(
        ...
        agent = initialize_agent(
            lc_tools, _get_lc_llm(), agent=AgentType.ZERO_SHOT_REACT_DESCRIPTION, verbose=True
        )
        return agent
    except ImportError:
        raise ImportError("LangChain not installed. Run: pip install langchain")
...
def import_retriever(retriever_type: str, *, source: str = None, **kwargs) -> Any:
    """
...
def _get_lc_llm():
    """Get LangChain LLM using openstackai configuration."""
    import os
    ...
    try:
        ...
        return ChatOpenAI(
            model=os.environ.get("AZURE_OPENAI_DEPLOYMENT", "gpt-4o-mini"), temperature=0.7
        )
    except ImportError:
...
def _create_stub_tool(name: str) -> LangChainTool:
    ...
    return LangChainTool(
        name=name, description=f"LangChain tool: {name} (not available)", func=stub_func
    )
