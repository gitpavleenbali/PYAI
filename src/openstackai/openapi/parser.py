...
def parse_openapi(source: Union[str, Path, Dict], base_url: Optional[str] = None) -> OpenAPISpec:
    ...
    return OpenAPISpec(
        ...
        security=spec_dict.get(
            "securityDefinitions", spec_dict.get("components", {}).get("securitySchemes", {})
        ),
        servers=spec_dict.get("servers", []),
    )
