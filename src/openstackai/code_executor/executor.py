            ...
            if isinstance(node, ast.Import):
                for alias in node.names:
                    ...
                    if (
                        ...
                        and module_name not in self.allow_modules
                    ):
...
class SafeExecutor(CodeExecutor):
    ...
    def _create_safe_globals(self) -> Dict[str, Any]:
        ...
        safe_builtins = {
            name: getattr(builtins, name) for name in self.SAFE_BUILTINS if hasattr(builtins, name)
        }
...
class PythonExecutor(SafeExecutor):
    ...
    def __init__(self, timeout: float = 30.0):
        super().__init__(timeout=timeout, allow_modules=["numpy", "pandas", "scipy", "sklearn"])
...
def execute_python(code: str, timeout: float = 30.0, safe: bool = True) -> ExecutionResult:
    """Execute Python code and return result.

    One-liner code execution:

        from openstackai.code_executor import execute_python
...
def execute_shell(
    command: str, timeout: float = 30.0, cwd: Optional[str] = None
) -> ExecutionResult:
    ...
    try:
        ...
        return ExecutionResult(
            success=False, error=str(e), execution_time_ms=(time.perf_counter() - start_time) * 1000
        )
