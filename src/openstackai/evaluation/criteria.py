...
class EvalCriteria(ABC):
    ...
    @abstractmethod
    def evaluate(
        self, actual: str, expected: Optional[str] = None, context: Optional[Dict[str, Any]] = None
    ) -> CriteriaResult:
...
class ExactMatch(EvalCriteria):
    ...
    def __init__(
        self, ignore_case: bool = False, ignore_whitespace: bool = False, strip: bool = True
    ):
    ...
    def evaluate(
        ...
        if expected is None:
            return CriteriaResult(
                passed=False, score=0.0, reason="No expected value provided for exact match"
            )
...
class ContainsMatch(EvalCriteria):
    ...
    def evaluate(
        ...
        if not substrs:
            return CriteriaResult(passed=True, score=1.0, reason="No substrings to check")
...
class NotContainsMatch(EvalCriteria):
    ...
    def __init__(self, forbidden: Optional[List[str]] = None, ignore_case: bool = True):
    ...
    def evaluate(
        ...
        if not forbidden:
            return CriteriaResult(passed=True, score=1.0, reason="No forbidden strings to check")
        ...
        return CriteriaResult(
            passed=passed,
            score=max(0.0, score),
            reason=f"Found {len(found_forbidden)} forbidden strings"
            if found_forbidden
...
class RegexMatch(EvalCriteria):
    ...
    def evaluate(
        ...
        if not pattern:
            return CriteriaResult(passed=False, score=0.0, reason="No regex pattern provided")
        ...
        try:
            ...
            return CriteriaResult(
                passed=False, score=0.0, reason=f"Invalid regex: {e}", details={"error": str(e)}
            )
...
class JSONSchema(EvalCriteria):
    ...
    def evaluate(
        ...
        if not schema:
            return CriteriaResult(passed=False, score=0.0, reason="No JSON schema provided")
        ...
        try:
            ...
            return CriteriaResult(
                passed=False, score=0.0, reason=f"Invalid JSON: {e}", details={"error": str(e)}
            )
    ...
    def _validate_schema(self, data: Any, schema: Dict[str, Any], path: str = "") -> tuple:
...
class SemanticSimilarity(EvalCriteria):
    ...
    def __init__(self, threshold: float = 0.8, model: str = "all-MiniLM-L6-v2"):
    ...
    def evaluate(
        ...
        if not expected:
            return CriteriaResult(
                passed=False, score=0.0, reason="No expected value for semantic comparison"
            )
        ...
        try:
            ...
            return CriteriaResult(passed=False, score=0.0, reason=str(e), details={"error": str(e)})
        except Exception as e:
            return CriteriaResult(
                passed=False,
                score=0.0,
...
class LLMJudge(EvalCriteria):
    ...
    def __init__(
        self, prompt: Optional[str] = None, model: str = "gpt-4o-mini", rubric: Optional[str] = None
    ):
...
"""
            ...
            passed = any(
                ...
                for word in ["yes", "correct", "good", "passed", "true"]
            )
            ...
            return CriteriaResult(
                passed=False, score=0.0, reason=f"LLM judge failed: {e}", details={"error": str(e)}
            )
...
class CustomCriteria(EvalCriteria):
    ...
    def __init__(
        self, func: Callable[[str, Optional[str], Optional[Dict]], tuple], name: str = "custom"
    ):
        self.func = func
        self.name = name
