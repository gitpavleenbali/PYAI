...
class TelecomAgents:
    ...
    @staticmethod
    def plan_advisor(*, carrier_name: str = "TeleCom", plans: List[Dict[str, Any]] = None):
        ...
        default_plans = plans or [
            {
                ...
                "data": "5GB",
                "price": "$35",
                "features": ["Unlimited calls", "Unlimited texts"],
            },
            {
                "name": "Standard",
                "data": "15GB",
            ...
            },
            {
                ...
                "data": "Unlimited",
                "price": "$85",
                "features": ["Unlimited everything", "5G", "Hotspot", "International"],
            },
        ]
...
- Offer callback if issue persists"""
    ...
    @staticmethod
    def retention_agent(*, max_discount: int = 20, can_offer_upgrade: bool = True):
...
class HealthcareAgents:
    ...
    def appointment_scheduler(
        *, facility_name: str = "HealthCare Clinic", departments: List[str] = None
    ):
...
class FinancialAgents:
    ...
    @staticmethod
    def banking_assistant(*, bank_name: str = "Financial Bank", products: List[str] = None):
...
class EcommerceAgents:
    ...
    @staticmethod
    def shopping_assistant(*, store_name: str = "Online Store", categories: List[str] = None):
...
- Document all issues"""
    ...
    @staticmethod
    def returns_agent(*, return_window: int = 30, free_returns: bool = True):
...
- Use overly technical language"""
    ...
    @staticmethod
    def course_advisor(*, institution: str = "University", programs: List[str] = None):
...
def list_industries() -> Dict[str, List[str]]:
    ...
    for name, industry in ALL_INDUSTRIES.items():
        agents = [
            a for a in dir(industry) if not a.startswith("_") and callable(getattr(industry, a))
        ]
        result[name] = agents
    return result
