...
class SessionManager:
    ...
    def __init__(
        self, backend: Optional[str] = None, store: Optional[BaseSessionStore] = None, **kwargs
    ):
    ...
    def _detect_backend(self) -> str:
        ...
        explicit = os.environ.get("pyai_SESSION_BACKEND")
        if explicit:
            return explicit.lower()

        # Check for Redis
        redis_host = os.environ.get("REDIS_HOST") or os.environ.get("pyai_REDIS_HOST")
        if redis_host:
            return "redis"

        # Check for SQLite path
        sqlite_path = os.environ.get("pyai_SESSION_DB")
        if sqlite_path:
            return "sqlite"
    ...
    def _create_store(self, backend: str, **kwargs) -> BaseSessionStore:
        ...
        if backend == "memory":
            ...
            db_path = kwargs.get("db_path") or os.environ.get("pyai_SESSION_DB", "pyai_sessions.db")
    ...
    def create(
        ...
        **kwargs,
        ...
        session = Session(id=session_id, user_id=user_id, agent_id=agent_id, **kwargs)
...
def get_session(
    session_id: str, user_id: Optional[str] = None, agent_id: Optional[str] = None
) -> Session:
